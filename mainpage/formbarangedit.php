...
include("configgabung.php");
    $id = $_GET['id'];
    $query = pg_query("SELECT * FROM barang WHERE id=$id");
    $barang = pg_fetch_array($query);
    $nb = $barang['nama_barang'];
    $jb = $barang['jenis_barang'];
    $desk = $barang['deskripsi'];
    $sk = $barang['status_kehilangan'];
    $tk = $barang['tanggal_kehilangan'];
    $temk = $barang['tempat_kehilangan'];
    $cp = $barang['contact_person'];
?>
...
<div class="upload-page-5Kf">
  <p class="upload-items-yvq">EDIT ITEMS</p>
  <div class="rectangle-24-6Em">
  </div>
<form action="prosesbarangedit.php" method="POST" enctype="multipart/form-data">
<input type="hidden" value="<?php echo $_GET['id'] ?>" name="id">
  ...
  <a class="rectangle-29-rky" href="daftarbarang.php">
    <button type="submit" name="edit" value="daftar" class="uploadbutton">Upload!</button>
  </a>
</form>
